...
{
    public partial class ccEstadoVale : System.Web.UI.Page
    {
        ...
        protected void Page_LoadComplete(object sender, EventArgs e)
        {
            ...
            if (IsPostBack == false)
            {
                ...
                pedidoLN.gridEstadoVale(gridEstado, pedidoEN);
            }
        ...
        protected void gridEstado_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (HttpUtility.HtmlDecode(row.Cells[5].Text) == "Aprobado")
                {
                    pedidoLN = new PedidoLNBorrar();
                    pedidoEN = new PedidoENBorrar();
                    pedidoEN.ccidVale = Convert.ToInt32(gridEstado.SelectedValue);

                    DataTable tabla, tabladetalle;
                    tabla = pedidoLN.rptVale(pedidoEN);
                    pedidoEN.ccidVale = Convert.ToInt32(gridEstado.SelectedValue);
                    tabladetalle = pedidoLN.rptValeDetalle(pedidoEN);

                    DataSet tablas = new DataSet();
                    tablas.Tables.Add(tabla);
                    tablas.Tables[0].TableName = "dtccVale";

                    tablas.Tables.Add(tabladetalle);
                    tablas.Tables[1].TableName = "dtccValeDetalle";

                    if (tabla.Rows.Count > 0)
                    {


                        crccVale cr = new crccVale();
                        cr.SetDataSource(tablas);



                        btnImprimir.Attributes.Add("onclick", "javascript:window.open('" + reportePdf("Vale", cr) + "','Vale'," +
