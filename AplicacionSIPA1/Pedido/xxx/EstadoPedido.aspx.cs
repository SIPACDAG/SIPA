...
namespace AplicacionSIPA1.Pedido
{
    public partial class EstadoPedido : System.Web.UI.Page
    {
        ...
        protected void Page_LoadComplete(object sender, EventArgs e)
        {
            ...
            //    {
            //        Label lblIDNomina = default(Label);
            //        lblIDNomina = (Label)PreviousPage.FindControl("lblIDNomina");
            //        ViewState["idNomina"] = lblIDNomina.Text;
            //         Label1.Text = Convert.ToString(ViewState["idNomina"]);
            //    }
            //}
            //catch (Exception ex)
            //{
            ...
            try
            {
                ...
                if (HttpUtility.HtmlDecode(row.Cells[5].Text) == "Aprobado")
                {
                    ...
                    if (tabla.Rows.Count > 0)
                    {
                        ...
                        crPedido cr = new crPedido();
                        cr.SetDataSource(tablas);
                                    


                        btnImprimir.Attributes.Add("onclick", "javascript:window.open('" + reportePdf("Pedido", cr) + "','Pedido'," +
