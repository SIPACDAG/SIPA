    ...
    public partial class VoBoN2 : System.Web.UI.Page
    {
        ...
        protected void Page_LoadComplete(object sender, EventArgs e)
        {
            if (IsPostBack == false)
            {
                try
                {
                    ...
                    planOperativoLN.DdlUnidadesxAnalista(ddlUnidades, usuario, anioActual);
        ...
        protected void ddlUnidades_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    planOperativoLN = new PlanOperativoLN();
                    planOperativoLN.DdlDependenciasxAnalista(ddlDependencias,Session["usuario"].ToString(),anio, idUnidad);
                }
        ...
        protected void ddlAnios_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                planOperativoLN = new PlanOperativoLN();
                planOperativoLN.DdlUnidadesxAnalista(ddlUnidades, Session["usuario"].ToString(), anio);
        ...
        protected bool validarPoa(int idUnidad, int anio)
        {
            ...
            try
            {
                ...
                //4	Revisión Analista POA
                else if (estadoPoa.Equals("4"))
                {
                    lblErrorPoa.Text = lblError.Text = string.Empty;
                ...
                //2	Revisión Subgerencia, 7	Revisión Dirección
                else if (estadoPoa.Equals("2") || estadoPoa.Equals("7"))
                {
                    lblErrorPoa.Text = lblError.Text = "El CUADRO DE MANDO INTEGRAL seleccionado se encuenta en estado: " + lblEstadoPoa.Text + ". No se puede Aprobar o Rechazar";
        ...
        protected void ddlDependencias_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (idUnidad > 0)
                {
                    ...
                    {
                        planOperativoLN.DdlDependenciasxAnalista(ddlJefaturaUnidad, Session["usuario"].ToString(), anio, idUnidad);
                    }
        ...
        protected void btnAprobar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                txtObser.Text = string.Empty;
                cambiarEstado(7, "APROBADA");
                EnvioDeCorreos objEC = new EnvioDeCorreos();
                objEC.EnvioCorreo(planOperativoLN.ObtenerCorreo(int.Parse(ddlUnidades.SelectedValue), 29), " Nuevo CMI APROBADO por Analista", " Nuevo CMI numero " + lblIdPoa.Text + ", " + lblSuccess.Text, Session["usuario"].ToString());

                presupuestoLN.InsertarBitacora(Session["usuario"].ToString(), ddlUnidades.SelectedValue, "ip", "APROBADO CMI Analista", txtObser.Text, 0, 0);
            }
            catch (Exception ex)
            {
        ...
        protected void btnRechazar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    cambiarEstado(6, "RECHAZADA");
                    presupuestoLN.InsertarBitacora(Session["usuario"].ToString(), ddlUnidades.SelectedValue, "ip", "RECHAZO CMI Analista", txtObser.Text, 0, 0);
                }
        ...
        protected void cambiarEstado(int idEstado, string nombreEstado)
        {
            try
            {
                ...
                validarPoa(Convert.ToInt32( ddlUnidades.SelectedValue), Convert.ToInt32(ddlAnios.SelectedValue));
                planOperativoLN = new PlanOperativoLN();
                int anio = int.Parse(ddlAnios.SelectedValue);
                ...
                DataSet dsResultado = planOperativoLN.ActualizarEstadoPoa(int.Parse(lblIdPoa.Text), idEstado, anio, null, "", usuario, observaciones,ip[0],ip[1],ip[2],"VoBoN2", " AplicacionSIPA1.Operativa.VoBoN2.btnAceptar()");
                ...
                //validarPoa(int.Parse(ddlUnidades.SelectedValue), anio);
            }
            catch (Exception ex)
            {
        ...
        protected void btnCodificar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Session.Add("idPoa", lblIdPoa.Text);
                    string _open = "window.open('CodificarPoa.aspx', '_newtab');";
                    ScriptManager.RegisterStartupScript(this, this.GetType(), Guid.NewGuid().ToString(), _open, true);
                }
                else
                    lblError.Text = "POA Inválido...";
            }
            catch (Exception ex)
            {
                lblError.Text = "btnCodificar_Click(). " + ex.Message;
            }
        }
